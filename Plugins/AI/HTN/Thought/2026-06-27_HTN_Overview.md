...
```
HandleEnemy                    ← Compound Task
├── Method A: RangedAttack     ← Method Precondition: "现在该用远程策略吗?"
│   ├── EnsureAmmo             ← SubTask (本身是 Compound Task)
│   ├── MoveToCover            ← SubTask (本身是 Primitive Task)
│   │   └── Operator: OpMoveToCover  ← Operator Precondition: "这个动作能做吗?"
│   └── ShootEnemy
│       └── Operator: OpShootEnemy   ← Operator Precondition
│
└── Method B: MeleeAttack      ← Method Precondition: "现在该用近战策略吗?"
    ├── MoveToEnemy
    └── MeleeHit
```
...
| 类型 | 回答的问题 | 计算对象 | 失败影响 |
| --- | --- | --- | --- |
| Method Precondition | 这个策略适不适合现在用 | 一组 SubTask 的整体可行性前提 | 跳过这个 Method，尝试下一个 |
| SubTask Precondition | 这个 SubTask 是 Compound Task 时，通过它自己的 Method Preconditions 体现；是 Primitive Task 时,通过 Operator Preconditions 体现 | 不是独立概念 | 见下文 |
| Operator Precondition | 这个原子动作当前能不能执行 | 一个具体动作的执行前提 | 当前分支失败,回溯到上层 Method |
...
判断 **"当前世界状态下,这套分解策略是否适用"**

每个 Compound Task 通常有多个 Method(分解策略),Method Preconditions 是这些策略之间的选择闸门

Planner 按顺序检查每个 Method 的 Preconditions,选第一个通过的来分解

```cpp
HandleEnemy (Compound Task)
  Method A: RangedAttack
    Precondition: EnemyVisible == true && WeaponReady == true
    SubTasks: EnsureAmmo -> MoveToCover -> ShootEnemy

  Method B: MeleeAttack
    Precondition: DistanceToEnemy < 200
    SubTasks: MoveToEnemy -> MeleeHit

  Method C: Flee
    Precondition: HP < 30
    SubTasks: FindEscapeRoute -> RunAway
```

每个 Method Precondition 回答的是:

- A: "现在适合用远程战术吗?" — 看到了敌人且武器就绪
- B: "现在适合用近战战术吗?" — 距离够近
- C: "现在应该逃跑吗?" — 血量危险

不要在 Method Precondition 里检查每个子任务自己能不能做
...
Method A: RangedAttack
  ...
  Precondition:
    EnemyVisible == true
    && WeaponReady == true
    && (Ammo >= 3 || CanReload == true)     // 这是 EnsureAmmo 该判断的
    && HasCover == true                      // 这是 MoveToCover 该判断的
    && EnemyHP > 0                           // 这是 ShootEnemy 该判断的
```
...
Method A: RangedAttack
  Precondition:
    EnemyVisible == true && WeaponReady == true  // 只判断"策略适用性"
```

把子任务的条件全部上移到 Method 会带来四个问题

1. 复用差: `MoveToCover` 单独使用时,它的条件已经在 `Operator` 上检查过一次了, `Method` 重复写一遍是冗余
2. 回溯信息混乱: `Method` 失败时,说 "Ammo < 3 失败", 但根本原因可能是别的 `Method` 也可以处理换弹
3. 改一处影响一片: `ShootEnemy` 改了条件,所有引用它的 `Method` 都要同步改
4. 失去 HTN 的分层抽象优势: `Method` 应该表达 "**高层策略**" ,不应该陷入子任务细节

#### SubTask Precondition

> 子任务前置条件

"SubTask Precondition" 是一个逻辑视角——指"作为某个 Method 的子任务出现时,这个 SubTask 自己应该判断什么"

| SubTask 类型 | 实际上是什么 |
| --- | --- |
| Compound Task | 它自己的所有 Method Preconditions |
| Primitive Task | 它绑定的 Operator 的 Preconditions |

这是为了分情况使用 `Method Precondition` 和 `Operator Precondition`

单独定义一个 `SubTask Precondition` 因为这是设计师最容易犯错的位置。设计师在写 Method 的时候,容易下意识地把"子任务需要的条件"也写到 Method 上。强调"SubTask 的条件归 SubTask 自己管"是为了让设计师养成关注点分离的习惯

比如下面这个例子，将所有的条件全部堆到 Method 上
...
- 如果 `CanReload` 后来变成 `false`,整个 `RangedAttack` 失败,但实际可能 `Ammo` 已经够了,根本不需要换弹
- `EnsureAmmo` 离开这个 Method 就没法复用

可以修改为下面这种方案

```cpp
Method A: RangedAttack
  Precondition: EnemyVisible && WeaponReady          ← Method 层:策略适用性
  SubTasks: EnsureAmmo -> MoveToCover -> ShootEnemy

  EnsureAmmo (Compound Task)
    Method A1: AlreadyHaveAmmo
      Precondition: Ammo >= 3                        ← SubTask 自己的 Method 层
      SubTasks: (空)
    Method A2: Reload
      Precondition: Ammo < 3                         ← SubTask 自己的 Method 层
      SubTasks: ReloadWeapon
        Operator: OpReloadWeapon
          Precondition: CanReload == true            ← Operator 层

  MoveToCover (Primitive Task)
    Operator: OpMoveToCover
      Precondition: HasCover == true                 ← Operator 层

  ShootEnemy (Primitive Task)
    Operator: OpShootEnemy
      Precondition: Ammo >= 3 && EnemyHP > 0         ← Operator 层
```

好处是

- `EnsureAmmo` 在任何其他 Method 里复用,条件都跟着它走
- `RangedAttack Method` 自身简洁,只关心"远程策略是否适用"

SubTask Precondition 不是 Method 写的, 是 SubTask 自己写在它的 Method/Operator 里的。

#### Operator Precondition

> 具体操作的前置条件
 
判断 **"这个原子动作现在能不能执行"。这是 HTN 里最贴近实际世界** 的条件检查

Operator Precondition 实际上有两个语义层次
1. 计划期 Precondition (Plan Precondition)
   - 用模拟世界状态 SimWorldState 计算
   - Planner 在递归到 Primitive Task 时检查
   - 失败 -> 当前分支失败,回溯到上层 Method
   - 必须是纯数据查询,快速、确定、可重复。例如：`Ammo >= 3`、`HasCover == true`、`EnemyHP > 0`
2. 执行期 Precondition (RunTime Precondition)
   - 用 **真实世界状态** 计算（查询 Actor、射线检测、感知组件等）
   - 失败 → Operator 立刻返回 Failed, 触发重规划

`Plan Precondition` 计算的是 在我**预测的世界状态**下,这个动作的前置资源/前置状态是否满足

```cpp
Operator: OpShootEnemy
  PlanPreconditions:
    Ammo >= 3              ← 我的预测 Ammo 是不是够 3 发?
    EnemyHP > 0            ← 我的预测里敌人还活着吗?
    EnemyVisible == true   ← 我的预测里能看见敌人吗?
  Effects:
    Ammo -= 3
    EnemyHP -= 50
```

> 这里的 Ammo、EnemyHP 都是 SimWorldState 里的值,不是真实的当前值

`RunTime Precondition` 计算的是 在真实当下,这个动作还能不能成功启动

```cpp
Operator: OpShootEnemy
  RuntimePreconditions: (或在 StartOperator 里实现)
    - Owner Pawn 仍然活着
    - Target Actor 仍然 Valid
    - 武器组件没有被卸下
    - Target 当前 LineOfSight 真的可达(规划期可能用粗略可见性,执行期做精确射线)
```
...
计划期 Precondition 失败:
  → 当前分支失败
  → 回溯到上层 Compound Task,尝试其他 Method
  → 完全不影响真实世界

执行期 Precondition 失败:
  → 当前 Operator 立刻 Abort
  → UHTNComponent 触发 OnPlanFailed
  → 根据策略重规划(全量或局部)
  → 真实世界可能已经发生了部分变化(动画播了一半、移动了一段距离)
```

#### Precondition 总结
| 维度 | Method  Precondition| Operator  Plan-Precondition| Operator  Runtime-Precondition | 
| --- | --- | --- | --- |
| 回答的问题 | 这个策略该不该用 ?| 这个动作 (预测中)能不能做?| 这个动作 (真实当下)能不能启动? | 
| 计算时机 | 规划期 ,选 Method 时| 规划期 ,展开 Primitive Task 时| 执行期 ,Operator.Start() 时 | 
| 数据来源 | SimWorldState | SimWorldState | 真实  Actor / 感知 / 物理查询 | 
| 失败影响 | 跳过此  Method,尝试下一个| 当前分支失败 ,回溯到上层 Method| Operator  Abort,触发重规划|  
| 性能要求 | 必须快 (每次规划都跑)| 必须快 (每次规划都跑)| 中等 (只在执行启动时跑一次) | 
| 实现方式 | 数据化 (FHTNCondition)| 数据化 (FHTNCondition)| C ++/蓝图运行时逻辑 | 
| 典型内容 | 环境状态、策略适用性 | 资源 /状态前提 | 真实世界查询、Actor 有效性 |
